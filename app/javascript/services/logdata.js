		...
		Process: function(logObject, i) {
			...
			for (var i = 0; i < arrayOfLogs.length; i++) {
				...
				for (var x = 0; x < arrayOfFields.length; x++) {
					...
					if (fieldSplit[1]) {
						if (fieldSplit[1][fieldSplit[1].length - 1] === ']') {
							fieldSplit[1] = fieldSplit[1].substring(1, (fieldSplit[1].trim().length - 2));
						} else {
							fieldSplit[1] = fieldSplit[1].substring(1, (fieldSplit[1].trim().length - 1));
						}
						var obj = {};
						obj[fieldSplit[0].trim()] = fieldSplit[1].trim();
						arrayOfFields[x] = obj;
					} else {
						arrayOfFields[x] = fieldSplit[0].substring(1, (fieldSplit[0].trim().length - 1));
					}
				}
				arrayOfFields[0] = {
				...
				flat.serial = arrayOfLogs[i] = flat;
			}
			return arrayOfLogs;
